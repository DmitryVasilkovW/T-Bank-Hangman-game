...
public class HangmanContextFactory implements HangmanGameContextFactory {
    ...
    @Override
    public HangmanGameContext createHangmanGameContext(int attempts, String expectedWord, String text) {
        var service = new HangmanStateServiceImpl();
        var word = new Word(expectedWord);
        var hangman = new char[28];
        var hangmanRec = new Hangman(hangman);
        var context = new HangmanGameContext(attempts, word, hangmanRec, new Word(text));
